...
import { Send, Mic, Volume2, Sparkles, User, Bot, Trash2, Copy, Download, Brain, Lightbulb, Zap } from 'lucide-react';
...
const initialMessage = {
  role: 'assistant',
  content: "Hello! I'm your AI Study Buddy. I can help you explain topics, create revision notes, generate quizzes, and keep you motivated.",
  ...
  const sendMessage = async (customText = input) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: response.data.response || response.data.message || 'I could not generate a response.',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } catch (error) {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: 'Your AI backend is not reachable right now. Check that your server is running and that /api/ai/chat exists.',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } finally {
    ...
    <div className="flex flex-col h-[calc(100vh-160px)] space-y-4">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-violet-600 rounded-xl shadow-lg shadow-violet-600/20"><Bot className="text-white" size={20} /></div>
          <div>
            <h2 className="text-2xl font-bold">AI Study Buddy</h2>
            <p className="text-xs text-emerald-400 font-bold uppercase tracking-widest">Neural Link Active</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={downloadChat} className="p-2 rounded-xl bg-white/5 border border-white/10 text-slate-500 hover:text-white transition-all"><Download size={18} /></button>
          <button onClick={clearChat} className="p-2 rounded-xl bg-white/5 border border-white/10 text-slate-500 hover:text-red-400 transition-all"><Trash2 size={18} /></button>
        </div>
      </header>
      ...
      <div className="flex-1 glass-card overflow-hidden flex flex-col relative border-white/5">
          ...
          {messages.map((msg, index) => (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex gap-3 max-w-[85%] md:max-w-[70%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-1 ${msg.role === 'user' ? 'bg-cyan-500' : 'bg-violet-600'}`}>
                  {msg.role === 'user' ? <User size={16} className="text-white" /> : <Bot size={16} className="text-white" />}
                </div>
                <div>
                  <div className={`p-4 rounded-2xl break-words ${msg.role === 'user' ? 'bg-gradient-to-br from-cyan-600 to-cyan-700 text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-slate-200 rounded-tl-none'}`}>
                  ...
                  <span className={`text-[9px] text-slate-500 font-bold mt-1.5 block ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>{msg.timestamp}</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="px-4 md:px-6 pb-3 flex flex-wrap gap-2">
          {quickPrompts.map((prompt) => {
            ...
            return (
              <button key={prompt.text} onClick={() => sendMessage(prompt.text)} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-slate-300 border border-white/5">
                <Icon size={14} />
                {prompt.text}
              </button>
            );
        ...
        <div className="p-4 md:p-6 bg-[#161B2E]/80 border-t border-white/5">
          <div className="flex items-center gap-3">
            <button onClick={toggleRecording} className={`p-3 rounded-xl border border-white/10 transition-all ${isRecording ? 'bg-red-500 text-white' : 'bg-white/5 text-slate-400 hover:text-white'}`}>
              <Mic size={18} />
            </button>
            <input
              value={input}
            ...
            />
            <button onClick={() => sendMessage()} disabled={!input.trim() || isTyping} className="p-3 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white">
              <Send size={18} />
            </button>
          </div>
        </div>
